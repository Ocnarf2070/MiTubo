create or replace view V_INACTIVO as
  (SELECT U.ALIAS, MAX(V.FECHA_CREACION)AS ULTIMOVIDEO, MAX(C.FECHA)AS ULTIMOCOMENTARIO, MAX(H.TERMINO)AS ULTIMABUSQUEDA  FROM USUARIO U,VIDEO V,CANAL CA,COMENTARIO C,HISTORIAL H 
